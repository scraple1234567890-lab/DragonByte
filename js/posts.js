...
function subscribeToRealtime() {
  if (postsContainer && !postsChannel) {
  ...
  if (articlesContainer && !articlesChannel) {
...
function wireAvatarRefresh() {
  window.addEventListener("storage", (event) => {
    ...
    if (postsContainer) renderPosts(postsCache);
    applyArticleSearch();
  });
...
async function init() {
  ...
  const loaders = [];
  if (postsContainer) loaders.push(loadPosts());
  if (articlesContainer) loaders.push(loadArticles());

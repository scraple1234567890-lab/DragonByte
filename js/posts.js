...
function renderArticles(articles, query = "") {
  ...
  // Accordion helper:
  // Close *any* expanded article card in the list. We check multiple signals
  // (class, aria-expanded, and `hidden`) because different code paths or cached
  // markup can occasionally leave them out of sync.
  const closeExpandedArticles = (exceptCard = null) => {
    if (!articlesContainer) return;
    ...
    cards.forEach((c) => {
      ...
      const toggleEl = c.querySelector(".articleToggle");
      const bodyEl = c.querySelector(".articleBody");

      const ariaOpen =
        toggleEl instanceof HTMLElement && toggleEl.getAttribute("aria-expanded") === "true";
      const hiddenOpen = bodyEl instanceof HTMLElement && bodyEl.hidden === false;
      const classOpen = c.classList.contains("isOpen");

      if (!ariaOpen && !hiddenOpen && !classOpen) return;
  ...
  articles.forEach((row) => {
    ...
    const handleToggle = () => {
      ...
      // Accordion behavior:
      // If we're opening this article, automatically minimize any other open one.
      if (open) closeExpandedArticles(card);

...
function renderArticles(articles, query = "") {
  ...
  articles.forEach((row) => {
    ...
    const metaText = document.createElement("p");
    metaText.className = "muted small articleMeta articlePreviewMeta";

    const authorName = getAuthorDisplayName(row);
    const articleStamp = formatShortDateTime(row.created_at);
    ...
    if (articleStamp) {
      ...
      sep.className = "articleMetaSep";
      sep.textContent = "•";

      const timeSpan = document.createElement("span");
      timeSpan.className = "articleMetaTime";
      timeSpan.textContent = articleStamp;

      metaText.append(sep, timeSpan);
      metaText.title = `${authorName} • ${articleStamp}`;
    } else {
      metaText.title = authorName;
    }

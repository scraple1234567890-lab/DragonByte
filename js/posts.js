...
function setArticleImagePreview(file) {
  ...
  if (!String(file.type || "").startsWith("image/")) {
    clearArticleImageUI();
    setStatus(articleStatus, "Please choose an image file (PNG, JPG, or WebP).", "error");
    return;
  }

  if (file.size > MAX_IMAGE_BYTES) {
    clearArticleImageUI();
    setStatus(articleStatus, "That image is too large. Please use an image under 5MB.", "error");
    return;
  }
...
function renderArticles(articles, query = "") {
  ...
  articles.forEach((row) => {
    ...
    excerpt.textContent = makeExcerpt(row.content, 220);
...
async function handleArticleSubmit(event) {
  ...
  if (selectedFile) {
    ...
    try {
      ...
      imageUrl = await uploadArticleCoverImage(selectedFile, title, currentUser.id);
    } catch (error) {
  ...
  const { error } = await supabase
    .from("articles")
    .insert([{ user_id: currentUser.id, title, content, image_url: imageUrl }]);
...
function wireArticleImageUpload() {
  ...
  if (articleImageClearButton instanceof HTMLButtonElement) {
    ...
    articleImageClearButton.addEventListener("click", () => {
      ...
      if (articleContentInput instanceof HTMLElement) {

...
/* =========================
   ...
   - Textarea: inserts \t (with multi-line indent/outdent via Shift+Tab)
   - Rich contenteditable editor: inserts 4 non-breaking spaces
   ========================= */
function enableTabInTextarea(textarea) {
  if (!textarea) return;

  const indent = "\t";
  const isLineStart = (value, pos) => pos === 0 || value[pos - 1] === "\n";
  ...
  textarea.addEventListener("keydown", (event) => {
    ...
    // If there is no selection, just insert a single tab.
    if (start === end) {
    ...
    // Multi-line selection: indent/outdent the selected lines.
    ...
    if (event.shiftKey) {
      // Outdent: remove one leading tab (or 2 leading spaces) per line when present.
      ...
      textarea.setSelectionRange(start, Math.max(lineStart, end + changed));
      return;
    }
    ...
    const nextBlock = indented.join("\n");
    textarea.value = value.slice(0, lineStart) + nextBlock + value.slice(lineEnd);
    textarea.setSelectionRange(start + indent.length, end + changed);
  });
}

function enableTabInRichEditor(editorEl) {
  if (!editorEl) return;

  const indent = "    "; // 4 spaces (works with CSS white-space: pre-wrap)
  ...
  editorEl.addEventListener("keydown", (event) => {
    ...
    const node = document.createTextNode(indent);
...
// Enable Tab indentation in editors.
enableTabInTextarea(postContentInput);
enableTabInRichEditor(articleEditor);

let postsCache = [];
let articlesCache = [];
const avatarCache = new Map();

// Article editing state (used on the Articles page + any page that renders articles)
let editingArticleId = null;
let editingArticleOriginalImageUrl = null;

// If the page is opened with ?article=<id>, automatically expand that article.
let requestedArticleId = null;
let requestedArticleOpened = false;
let requestedSearchQuery = null;
let requestedEditRequested = false;
let requestedEditOpened = false;

try {
  const params = new URLSearchParams(window.location.search);
  requestedArticleId = params.get("article");
  requestedSearchQuery = params.get("q") || params.get("query") || params.get("search") || null;
  const editParam = String(params.get("edit") || "").toLowerCase();
  requestedEditRequested = editParam === "1" || editParam === "true" || editParam === "yes";
} catch {
  requestedArticleId = null;
  requestedSearchQuery = null;
  requestedEditRequested = false;
}
...
function maybeOpenRequestedArticle() {
  if (!requestedArticleId || requestedArticleOpened) return;
  if (!articlesContainer) return;

  const card = articlesContainer.querySelector(`.articleCard[data-id="${cssEscape(requestedArticleId)}"]`);
  if (!card) return;

  const toggle = card.querySelector(".articleToggle[aria-controls]");
...
function maybeBeginEditRequestedArticle() {
  if (!requestedEditRequested || requestedEditOpened) return;
  if (!requestedArticleId) return;
  if (!currentUser) return;

  const row = (articlesCache || []).find((a) => String(a?.id) === String(requestedArticleId));
  if (!row) return;
  if (row.user_id !== currentUser.id) return;

  requestedEditOpened = true;
  beginEditArticle(row);

  if (articleComposerCard instanceof HTMLElement) {
    requestAnimationFrame(() => {
      articleComposerCard.scrollIntoView({ behavior: "smooth", block: "start" });
    });
  }
}
...
function setButtonBusy(button, busy, busyText, defaultText) {
  ...
  // Allow callers to override the "default" label (useful for modes like Edit vs Publish).
...
function sanitizeArticleHtml(dirtyHtml) {
  ...
  // Text alignment classes (saved on block elements)
  const allowedTextAlignClasses = new Set([
    'rt-text--left',
    'rt-text--center',
    'rt-text--right'
  ]);

  // Only allow these classes on block-ish tags (prevents random styling injection).
  const allowedTextClassTags = new Set(['p','div','h2','h3','h4','blockquote','li']);

  const filterTextClassList = (classValue) => {
    const raw = String(classValue || '');
    if (!raw.trim()) return '';
    const kept = raw
      .split(/\s+/)
      .map((c) => c.trim())
      .filter((c) => allowedTextAlignClasses.has(c));
  ...
  const cleanNode = (node) => {
    ...
    // Capture safe text alignment (convert style/align -> class)
    if (allowedTextClassTags.has(tag)) {
      const styleAttr = String(el.getAttribute('style') || '').toLowerCase();
      const alignAttr = String(el.getAttribute('align') || '').toLowerCase().trim();
      let detected = null;
      const match = styleAttr.match(/text-align\s*:\s*(left|center|right|start|end|-webkit-center)\b/);
      if (match) detected = match[1];
      if (detected === 'start') detected = 'left';
      if (detected === 'end') detected = 'right';
      if (detected === '-webkit-center') detected = 'center';
      if (!detected && (alignAttr === 'left' || alignAttr === 'center' || alignAttr === 'right' || alignAttr === 'start' || alignAttr === 'end')) detected = alignAttr;
      if (detected === 'start') detected = 'left';
      if (detected === 'end') detected = 'right';
      if (detected) {
        // Normalize: ensure only one alignment class is present.
        el.classList.remove('rt-text--left','rt-text--center','rt-text--right');
        el.classList.add(`rt-text--${detected}`);
      }
    }
    ...
    for (const attr of Array.from(el.attributes)) {
      ...
      // Allow safe alignment classes on block-ish tags.
      if (name === "class" && allowedTextClassTags.has(tag)) {
        const filtered = filterTextClassList(value);
        if (filtered) el.setAttribute("class", filtered);
        else el.removeAttribute("class");
        continue;
      }

      // Explicitly drop style + legacy align attrs (we convert align to classes above).
      if ((name === "style" || name === "align") && allowedTextClassTags.has(tag)) {
        el.removeAttribute(attr.name);
        continue;
      }
...
function setArticleImageHelp(text) {
  ...
  articleImageHelp.textContent = value;
  // Hide when empty so the composer stays clean.
  articleImageHelp.hidden = !value;
}
...
function setArticleImagePreview(file) {
  ...
  setArticleImageHelp(`Selected ${formatBytes(file.size)}.`);
}
...
async function deleteArticle(row, triggerButton) {
  ...
  try {
    ...
    // IMPORTANT:
    // Supabase/PostgREST can return success even when 0 rows were affected (e.g., RLS silently blocks).
    ...
    if (!Array.isArray(deletedRows) || deletedRows.length === 0) {
      const err = new Error(
        "Nothing was deleted (0 rows affected). This usually means Supabase Row Level Security blocked the delete."
      );
    ...
    // Also refresh in the background to keep cache honest (helps if realtime isn't enabled)
    loadArticles().catch(() => {});
  } catch (error) {
    ...
    const needsPolicyHint =
      lower.includes("permission") ||
      lower.includes("policy") ||
      lower.includes("rls") ||
      lower.includes("row level") ||
      lower.includes("nothing was deleted") ||
      lower.includes("0 rows") ||
      String(error?.code || "").toLowerCase().includes("no_rows");

    const hint = needsPolicyHint
      ? "\n\nSupabase tip: enable RLS + add a DELETE policy on the articles table like: auth.uid() = user_id"
      : "";
...
function resetArticleComposerToPublishMode() {
  editingArticleId = null;
  editingArticleOriginalImageUrl = null;

  // Restore header text.
  if (articleComposerCard instanceof HTMLElement) {
    const kicker = articleComposerCard.querySelector(".postComposerHeader .kicker");
    const sub = articleComposerCard.querySelector(".postComposerHeader .muted.small");

    // Keep the composer header clean in edit mode (no extra instructional text).
    // Restore whatever the default UI text is for "write" mode.
    if (kicker instanceof HTMLElement) {
      if (!kicker.dataset.defaultText) kicker.dataset.defaultText = kicker.textContent || "Write an article";
      kicker.textContent = kicker.dataset.defaultText;
    }
    if (sub instanceof HTMLElement) {
      if (!sub.dataset.defaultText) sub.dataset.defaultText = sub.textContent || "";
      sub.textContent = sub.dataset.defaultText;
    }
  }
// Restore submit label.
  if (articleSubmitButton instanceof HTMLButtonElement) {
    setButtonBusy(articleSubmitButton, false, "", "Publish");
  }
}

function setArticleComposerToEditMode(row) {
  if (!row?.id) return;

  editingArticleId = row.id;
  editingArticleOriginalImageUrl = row.image_url || null;
  ...
  if (articleComposerCard instanceof HTMLElement) {
    ...
    if (kicker instanceof HTMLElement) {
      if (!kicker.dataset.defaultText) kicker.dataset.defaultText = kicker.textContent || "Write an article";
      // Keep the header unchanged in edit mode.
      kicker.textContent = kicker.dataset.defaultText;
    }
    if (sub instanceof HTMLElement) {
      if (!sub.dataset.defaultText) sub.dataset.defaultText = sub.textContent || "";
  ...
  if (articleSubmitButton instanceof HTMLButtonElement) {
    setButtonBusy(articleSubmitButton, false, "", "Save changes");
  }

  // Helpful note about cover images.
  if (editingArticleOriginalImageUrl) {
    setArticleImageHelp("Cover image will stay the same unless you choose a new file.");
  }
}

function clearArticleComposerFields() {
...
function beginEditArticle(row) {
  if (!row?.id) return;

  if (!currentUser) {
    alert("Please log in to edit an article.");
    return;
  }
  if (row.user_id !== currentUser.id) {
  ...
  // Open the composer and load the article content.
  openComposer(articleComposerCard, shareArticleButton, articleTitleInput);
  setArticleComposerToEditMode(row);

  if (articleTitleInput) articleTitleInput.value = row.title || "";
  if (articleTagsInput) {
    const tags = getTagsFromRow(row);
    articleTagsInput.value = tags.length ? tags.join(", ") : "";
  }

  if (articleEditor instanceof HTMLElement) {
    const normalized = normalizeStoredContentToHtml(row.content || "");
    const safe = sanitizeArticleHtml(normalized);
    articleEditor.innerHTML = safe || "<p><br></p>";
  }

  // Clear any previously selected NEW cover image (we keep the existing unless user picks a new one).
  clearArticleImageUI();

  // No extra "editing" helper text; keep status area for real validation/errors.
  setStatus(articleStatus, "", "muted");
}
...
function applyArticleSearch() {
  ...
  maybeOpenRequestedArticle();
  maybeBeginEditRequestedArticle();
}
...
function renderArticles(articles, query = "") {
  ...
  const closeExpandedArticles = (exceptCard) => {
    if (!articlesContainer) return;
    const cards = articlesContainer.querySelectorAll(".articleCard");
  ...
  articles.forEach((row) => {
    ...
    // We now hide tags/excerpt in the collapsed (“small”) cards via CSS, so we no longer
    // need to reserve extra caption height per-card. Keep this class for styling hooks.
    if (hasTags) {
...
row2.append(author, controls);
    ...
    const handleToggle = () => {
      ...
      // Accordion behavior: if we're opening this article, automatically
      // minimize/close any other opened article on the page.
      if (shouldOpen) {
...
async function handleArticleSubmit(event) {
  ...
  const actionText = isEditing ? "Save changes" : "Publish";
  const busyText = isEditing ? "Saving..." : "Publishing...";

  setButtonBusy(articleSubmitButton, true, busyText, actionText);
  setStatus(articleStatus, "");

  // Keep existing cover image when editing unless a new file is selected.
  let imageUrl = isEditing ? (editingArticleOriginalImageUrl || null) : null;
  const previousImageUrl = editingArticleOriginalImageUrl || null;

  const selectedFile =
    articleImageInput instanceof HTMLInputElement ? articleImageInput.files?.[0] || null : null;

  if (selectedFile) {
    // Validate again at save time (preview validation could be bypassed).
    if (!String(selectedFile.type || "").startsWith("image/")) {
      setStatus(articleStatus, "Please choose an image file.", "error");
      setButtonBusy(articleSubmitButton, false, busyText, actionText);
      return;
    }
  ...
  try {
    if (isEditing) {
      // Update existing article (owner only).
      ...
      if (!Array.isArray(updatedRows) || updatedRows.length === 0) {
        const err = new Error(
          "Nothing was updated (0 rows affected). This usually means Supabase Row Level Security blocked the update."
        );
        err.code = "NO_ROWS_UPDATED";
        throw err;
      }

      // If we replaced the cover image, clean up the old one (non-fatal).
      if (selectedFile && previousImageUrl && previousImageUrl !== imageUrl) {
        tryRemoveArticleCoverImage(previousImageUrl).catch(() => {});
      }

      // Reset + refresh
      resetArticleComposerToPublishMode();
      clearArticleComposerFields();
      setStatus(articleStatus, "Changes saved!", "success");
      closeComposer(articleComposerCard, shareArticleButton);
      await loadArticles();
    } else {
    ...
    console.error(isEditing ? "Error updating article" : "Error creating article", error);
    ...
    const needsPolicyHint =
      ...
      lower.includes("rls") ||
      lower.includes("row level") ||
      lower.includes("nothing was updated") ||
      lower.includes("0 rows");

    const hint = needsPolicyHint
      ? " (Supabase tip: enable RLS + add an UPDATE policy on the articles table like: auth.uid() = user_id)"
      : lower.includes("tags") && lower.includes("column")
        ? " (Add an 'articles.tags' column as text[] or json in Supabase.)"
        : "";

    setStatus(articleStatus, (error?.message || "Could not save article. Please try again.") + hint, "error");
  } finally {
    setButtonBusy(articleSubmitButton, false, busyText, actionText);
  }
}
  ...
  if (shareArticleButton) {
    shareArticleButton.addEventListener("click", () => {
      ...
      if (articleComposerCard?.hidden) {
        // Opening the composer to create a NEW article should always be in Publish mode.
        if (editingArticleId) {
          resetArticleComposerToPublishMode();
          clearArticleComposerFields();
          setStatus(articleStatus, "");
      ...
      // If we were editing, clear everything so we don't accidentally publish edits as a new article.
      if (editingArticleId) {
        resetArticleComposerToPublishMode();
        clearArticleComposerFields();
      } else {
...
async function init() {
  // Pre-fill article search from URL (?q=...) if provided.
  if (requestedSearchQuery && articleSearchInput instanceof HTMLInputElement) {
    articleSearchInput.value = requestedSearchQuery;
    if (articleSearchPanel instanceof HTMLElement) {
      articleSearchPanel.hidden = false;
    }
    if (articleSearchToggleButton instanceof HTMLButtonElement) {

...
function formatDate(input) {
  ...
  // Short, readable date + time. Keeps layouts tidy on small screens.
  // Example: "Jan 5, 2026 • 1:23 PM"
...
function renderArticles(articles, query = "") {
  ...
  articles.forEach((row) => {
    ...
    // Meta line: smaller avatar tucked right beside the timestamp.
    const metaLine = document.createElement("div");
    metaLine.className = "articleMetaLine";
    const avatar = createAvatarElement(row.user_id);
    metaLine.append(avatar, metaText);
    ...
    textWrap.append(title, metaLine, excerpt);
    ...
    // Keep the right-side visuals (cover thumb + chevron) in their own container so
    // small screens can wrap/stack cleanly without overlapping the title/date.
    const metaRight = document.createElement("div");
    metaRight.className = "articleMetaRight";
    if (thumb) metaRight.appendChild(thumb);
    metaRight.appendChild(chevron);

    metaRow.append(textWrap, metaRight);
...
async function handleArticleSubmit(event) {
  ...
  const { error } = await supabase
    .from("articles")
    .insert([{ user_id: currentUser.id, title, content: contentHtml || "", image_url: imageUrl }]);

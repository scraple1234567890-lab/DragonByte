...
function renderUserArticles(articles) {
  ...
  articles.forEach((row) => {
    ...
    card.className = "card articleCard profileArticleCard";
    card.dataset.id = row.id;

    // Click anywhere on the preview to jump to Articles page and auto-open this article.
    const linkWrap = document.createElement("div");
    linkWrap.className = "articleToggle articleToggle--hero articleToggle--link";
    linkWrap.setAttribute("role", "link");
    linkWrap.tabIndex = 0;
    linkWrap.setAttribute("aria-label", `Open article: ${row.title || "Untitled"}`);

    const goToArticle = () => {
      try {
        window.location.href = `./articles.html?article=${encodeURIComponent(row.id)}`;
      } catch {
        window.location.href = "./articles.html";
      }
    };

    linkWrap.addEventListener("click", goToArticle);
    linkWrap.addEventListener("keydown", (event) => {
    ...
    if (hasTags) {
      ...
      tags.slice(0, 8).forEach((tag) => {
        ...
        chip.addEventListener("click", (event) => {
          // Don't navigate/open the article when clicking tags.
          event.preventDefault();
    ...
    openIcon.className = "articleChevron";
    openIcon.setAttribute("aria-hidden", "true");
    openIcon.textContent = "↗";
    controls.appendChild(openIcon);

...
function renderUserArticles(articles) {
  ...
  // Match the Articles page behavior: opening one article closes the others.
  ...
  articles.forEach((row) => {
    ...
    if (hasTags) {
      ...
      tags.slice(0, 8).forEach((tag) => {
        ...
        chip.addEventListener("click", (event) => {
          // Don't toggle when clicking tags.
    ...
    // Owner actions: hidden while collapsed via CSS, revealed once opened.
    ...
    bodyTop.className = "articleBodyTop";

    const openFull = document.createElement("a");
    openFull.className = "btn btnGhost btnSm";
    openFull.href = `./articles.html?article=${encodeURIComponent(row.id)}`;
    openFull.textContent = "Open on Articles page ↗";
    openFull.addEventListener("click", (e) => e.stopPropagation());

    bodyTop.appendChild(openFull);

    const body = document.createElement("div");
    body.className = "rtContent";
    body.innerHTML = sanitizedContent || "";

    bodyWrap.append(bodyTop, body);

...
// This needs to be resilient because header height can change after fonts/images load.
...
function syncSiteHeaderOffset() {
  ...
  document.documentElement.style.setProperty("--siteHeaderOffset", `${height}px`);
}

function initSiteHeaderOffsetSync() {
  const header = document.querySelector(".siteHeader");
  if (!(header instanceof HTMLElement)) return;

  const resync = () => {
    // two-phase measure helps when layout is still settling
  ...
  // If supported, resync when fonts finish loading.
  if (document.fonts?.ready) {
    document.fonts.ready.then(resync).catch(() => {});
  }

  // If supported, resync when the header itself changes size (menu, wrapping, etc.).
  if ("ResizeObserver" in window) {
    headerResizeObserver?.disconnect?.();
    headerResizeObserver = new ResizeObserver(resync);
